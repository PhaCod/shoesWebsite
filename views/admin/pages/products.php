<div class="admin-header">
    <h1>Sản Phẩm</h1>
    <a href="/shoesWebsite/views/admin/index.php?controller=adminProduct&action=addProduct" class="btn">Thêm Sản Phẩm Mới</a>
</div>

<!-- Form tìm kiếm -->
<div class="search-form" style="margin: 20px 0; text-align: center;">
    <form action="/shoesWebsite/views/admin/index.php?controller=adminProduct&action=products" method="get" style="display: inline-block;">
        ...
        <?php else: ?>
            <?php foreach ($products as $product): ?>
                <tr>
                    ...
                    <td>
                    <?php $imageUrl = !empty($product['image']) && filter_var($product['image'], FILTER_VALIDATE_URL) ? htmlspecialchars($product['image']) : '/shoesWebsite/public/placeholder.jpg';
                    ...
                    <td>
                        <a href="/shoesWebsite/views/admin/index.php?controller=adminProduct&action=editProduct&id=<?php echo $product['id']; ?>" class="btn-edit">Sửa</a>
                        <a href="/shoesWebsite/views/admin/index.php?controller=adminProduct&action=deleteProduct&id=<?php echo $product['id']; ?>" class="btn-delete" onclick="return confirm('Bạn có chắc muốn xóa sản phẩm này?')">Xóa</a>
                    </td>
                </tr>
            <?php endforeach; ?>
        <?php endif; ?>
...
<?php if (!empty($products) && $totalPages > 1): ?>
        <div class="pagination" style="margin: 20px 0; text-align: center;">
            <?php
            $baseUrl = '/shoesWebsite/views/admin/index.php?controller=adminProduct&action=products';
